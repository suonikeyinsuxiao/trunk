...
void convertRgb(int nFd, Bmp_S* psBmp, U16 uBpp)
{
	...
	//for(row = 0 ; row < psBmp->m_sBmpHeader.m_nImageHight; row++)//保存成.bmp文件
	for(row = psBmp->m_sBmpHeader.m_nImageHight-1 ; row >=0; row--)//直接写到framebuffer
	{
		for(col = 0 ; col < psBmp->m_sBmpHeader.m_nImageWidth; col++)
		{
			switch (uBpp)
