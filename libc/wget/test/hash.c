...
//	char m_acKey[64];
//	char m_acVal[64];
//	struct hash_table * m_pHashTable;
//};
...
main (void)
{
	...
	int size = 0;
	struct cell *c = NULL;
  	testfun_t equals = NULL;

	nRet = file_is_exist(INFO_PATH);
	if (-1 == nRet)
	{
	...
	info_fp = fopen(INFO_PATH, "r");

	while(-1 != (readLen = getline(&pcLine, &nLen, info_fp)))
	{
		char* pcStr1 = NULL;
		char acKey[64] = {0};
		char acValue[128] = {0};
		printf("pcLine=%p\n", pcLine);
		int nLineLen = strlen(pcLine);
		printf("string len=%d\n", nLineLen);
		printf("string:%s", pcLine);

		pcStr1 = strstr(pcLine, "=");
		//printf("len:%d\n", pcStr1-pcLine);
		strncpy(acKey, pcLine, pcStr1-pcLine);
		printf("key=%s\n", acKey);
		strncpy(acValue, pcStr1+1, nLineLen-1-(pcStr1-pcLine));
		printf("value=%s\n", acValue);

		hash_table_put(ht, strdup(acKey), strdup(acValue));
	}
  	size = ht->size;
	printf("all size:%d\n", size);
	...
	//print_hash(ht);
	//printf("work_mode:%s\n", hash_table_get(ht, "work_mode"));
	//printf("language_mode:%s\n", hash_table_get(ht, "language_mode"));
	//printf("sysvol:%s\n", hash_table_get(ht, "sysvol"));
	//printf("welcome_mode:%s\n", hash_table_get(ht, "welcome_mode"));
	//printf("softversion:%s\n", hash_table_get(ht, "softversion"));
	//printf("hardversion:%s\n", hash_table_get(ht, "hardversion"));
	free(pcLine);
	pcLine = NULL;

//	hash_table_put (ht, "frank", "1here I am!");
//	hash_table_put (ht, "jk", "2here I am!");
//	hash_table_put (ht, "lawson", "3here I am!");
//	hash_table_put (ht, "lasi", "4here I am!");
//	hash_table_put (ht, "aaa", "5here I am!");
//
//	printf("%s\n", hash_table_get(ht, "frank"));
//	printf("%s\n", hash_table_get(ht, "aaa"));
