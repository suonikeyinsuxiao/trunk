...
MSGQUEUEHANDLE createMsgQueue(int nMaxMsgNum)
{
	...
	for (i = 0; i < nMaxMsgNum; i++)
	{
		...
		psMsgQueCxt->m_psNodes[i].m_sMsg.m_pMsgData = NULL;
	}
...
int sendMsg(MSGQUEUEHANDLE psMsgQueCxt, MSG_S* psMsg)
{
	...
	switch(psMsg->m_eMsgType)
	{
		case e_INT:
			{
				psNode->m_sMsg.m_pMsgData = (int*)malloc(psMsg->m_nMsgLen*sizeof(int));
				memcpy(psNode->m_sMsg.m_pMsgData, psMsg->m_pMsgData, psMsg->m_nMsgLen*sizeof(int));
			}
			break;
		case e_LONG:
			{
				psNode->m_sMsg.m_pMsgData = (long*)malloc(psMsg->m_nMsgLen*sizeof(long));
				memcpy(psNode->m_sMsg.m_pMsgData, psMsg->m_pMsgData, psMsg->m_nMsgLen*sizeof(long));
			
			}
			break;
		case e_FLOAT:
			{
				psNode->m_sMsg.m_pMsgData = (float*)malloc(psMsg->m_nMsgLen*sizeof(float));
				memcpy(psNode->m_sMsg.m_pMsgData, psMsg->m_pMsgData, psMsg->m_nMsgLen*sizeof(float));
			
			}
			break;
		case e_DOUBLE:
			{
				psNode->m_sMsg.m_pMsgData = (double*)malloc(psMsg->m_nMsgLen * sizeof(double));
				memcpy(psNode->m_sMsg.m_pMsgData, psMsg->m_pMsgData, psMsg->m_nMsgLen*sizeof(double));
...
int recvMsg(MSGQUEUEHANDLE psMsgQueCxt, MSG_S* psMsg)
{
	...
	switch(psMsg->m_eMsgType)
	{
		case e_INT:
			{
				memcpy(psMsg->m_pMsgData, psNode->m_sMsg.m_pMsgData, psNode->m_sMsg.m_nMsgLen*sizeof(int));
				free(psNode->m_sMsg.m_pMsgData);
				psNode->m_sMsg.m_pMsgData = NULL;
			}
			break;
		case e_LONG:
			{
				memcpy(psMsg->m_pMsgData, psNode->m_sMsg.m_pMsgData, psNode->m_sMsg.m_nMsgLen*sizeof(long));
				free(psNode->m_sMsg.m_pMsgData);
				psNode->m_sMsg.m_pMsgData = NULL;
		...
		case e_FLOAT:
			{
				memcpy(psMsg->m_pMsgData, psNode->m_sMsg.m_pMsgData, psNode->m_sMsg.m_nMsgLen*sizeof(float));
				free(psNode->m_sMsg.m_pMsgData);
				psNode->m_sMsg.m_pMsgData = NULL;
		...
		case e_DOUBLE:
			{
				memcpy(psMsg->m_pMsgData, psNode->m_sMsg.m_pMsgData, psNode->m_sMsg.m_nMsgLen*sizeof(double));
				free(psNode->m_sMsg.m_pMsgData);
				psNode->m_sMsg.m_pMsgData = NULL;
			}
			break;
...
MSG_S* createMsg(int nId, int nMsgLen, MSGTYPE_E eMsgType)
{
	...
	switch(psMsg->m_eMsgType)
	{
		case e_INT:
			{
				psMsg->m_pMsgData = (int*)malloc(psMsg->m_nMsgLen*sizeof(int));
			}
			break;
		case e_LONG:
			{
				psMsg->m_pMsgData = (long*)malloc(psMsg->m_nMsgLen*sizeof(long));
			}
			break;
		case e_FLOAT:
			{
				psMsg->m_pMsgData = (float*)malloc(psMsg->m_nMsgLen*sizeof(float));
			}
			break;
		case e_DOUBLE:
			{
				psMsg->m_pMsgData = (double*)malloc(psMsg->m_nMsgLen*sizeof(double));
				LOGD(TAG, "psMsg->m_nMsgLen=%d, psMsg->m_pMsgData=%p\n",psMsg->m_nMsgLen ,psMsg->m_pMsgData);	
			}
			break;
		case e_STRING:
			{
