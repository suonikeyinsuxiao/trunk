...
int initEncoder(ENCODER_S* pcEncoder,int nColorSpaceType,  int nWidth, int nHeight)
{
	...
	pcEncoder->m_psParam->i_timebase_den = pcEncoder->m_psParam->i_fps_num;
	pcEncoder->m_psParam->i_timebase_num = pcEncoder->m_psParam->i_fps_den;
	pcEncoder->m_psParam->i_bframe = 5;
...
//			printf("X264_CSP_I420\n");
	...
	if (-1 == pcEncoder->m_nUDPSock)
	{
		printf("pcEncoder->m_nUDPSock == -1\n");	
		return -1;
	}

	int nSendLen =0;
	U32	nTimeout = 60*1000;

	for (i = 0; i < pcEncoder->m_nNal; ++i)
	{
//		printf("pcEncoder->m_psNal[%d].i_payload=%d\n",i,pcEncoder->m_psNal[i].i_payload);

		if (-1 == netSocketSendto(pcEncoder->m_nUDPSock, pcEncoder->m_psNal[i].p_payload, pcEncoder->m_psNal[i].i_payload, &nSendLen, pcEncoder->m_sServerUdp.m_acIp,SERVER_UDP_PORT,nTimeout))
		{
			fprintf(stderr, "send video data error:%s\n", strerror(errno));		
			return -1;
		}
